...
for ext in $ext_array; do
  if ! printf '%s\n' "${val_arr[@]}" | grep -qx "$ext"; then
    echo "$ext"
    echo "Not intended File Format 3"
    exit 1
  fi
done
...
if [[ "$array_size" -ne "2" ]]; then
  echo "Array size" 
  echo "Not intended File Format 7"
  exit 1
fi
...
val_arr=("true" "false")
  if ! printf '%s\n' "${val_arr[@]}" | grep -qx "$archived"; then
    echo "Not intended File Format 1"
    exit 1
  fi
...
for ext in $arc_list; do
  if ! printf '%s\n' "${val_arr[@]}" | grep -qx "$ext"; then
    echo "Not intended File Format 2"
    exit 1
  fi
done
